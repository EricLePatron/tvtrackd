...
function recomputeFromBatch(prevHome: HomeData, batch: MarkWatchedBatch): HomeData {
  ...
  // d'`onSettled`. Voir `markWatchedOnSuccess`/`markWatchedOnError`.
  ...
  //
  // Limite connue et acceptée : si DEUX mutations en vol touchent la MÊME
  // série a_voir et que l'une réussit pendant que l'autre échoue ensuite (en
  // étant la dernière à drainer le batch), ce recalcul peut brièvement
  // repasser la série en a_voir alors que le trigger SQL a réellement fini
  // par la faire passer en_cours (succès de l'autre mutation) — un flash
  // transitoire, auto-corrigé au prochain refetch (`onSettled` invalide
  // toujours `home-schedule`). Scénario rare (deux épisodes distincts de la
  // même série tapés en concurrence — surtout atteignable depuis le
  // calendrier) et à effet borné, dans la même catégorie que le
  // `heroProgress` masqué le temps d'une rotation.
...
/**
 * Retire UNIQUEMENT cet épisode du registre en vol, puis recalcule le
 * `HomeData` affiché à partir de `base` + ce qu'il reste dans `inFlight` —
 * l'optimisme de TOUT AUTRE épisode encore en vol (ajouté par un
 * `markWatchedOnMutate` concurrent) survit intact, puisqu'il est toujours
 * présent dans `inFlight` et repris par `recomputeFromBatch`. C'est le
 * correctif du bug de rollback par snapshot : aucun snapshot "avant cette
 * mutation" n'est jamais restauré.
 *
 ...
 * entièrement drainé).
 */
export function markWatchedOnError(qc: QueryClient, userId: string, episodeId: number): void {
  const batchMap = getBatchMap(qc);
...
/**
 * Bookkeeping uniquement — retire cet épisode du registre en vol, mais NE
 * réécrit PAS le cache : `base` précède le succès de cet épisode, le
 * recalculer à partir d'elle ici annulerait à tort l'épisode qui vient
 * justement de réussir. L'écriture optimiste déjà affichée reste telle
 * quelle jusqu'à ce que l'invalidation d'`onSettled` (voir `useMarkWatched`)
 * apporte le refetch faisant autorité peu après.
 */
export function markWatchedOnSuccess(qc: QueryClient, userId: string, episodeId: number): void {
  const batchMap = getBatchMap(qc);
  const batch = batchMap.get(userId);
  if (!batch) return;
 ...
 * `markWatchedOnError`/`markWatchedOnSuccess` ci-dessus (testables
 * directement contre un `QueryClient`, sans rendre ce hook) ; cette fonction
 * ne fait que les brancher sur le cycle de vie de `useMutation`.
 *
...
export function useMarkWatched() {
  ...
  return useMutation({
    ...
    onMutate: ({ episodeId, showId }) => {
      if (!user) return;
      markWatchedOnMutate(qc, user.id, { episodeId, showId });
    },
    onError: (_err, { episodeId }) => {
      toast.error("Impossible de marquer l'épisode");
      if (user) markWatchedOnError(qc, user.id, episodeId);
    },
    onSuccess: (_data, { episodeId }) => {
      if (user) markWatchedOnSuccess(qc, user.id, episodeId);
    },
