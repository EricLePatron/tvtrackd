...
describe("getTemporalBarClass", () => {
  ...
  it("returns the thicker, full-strength primary bar for today", () => {
    expect(getTemporalBarClass(TODAY, TODAY)).toBe("w-1 bg-primary");
  });

  it("returns a reduced-opacity cyan-accent bar for a past date", () => {
    expect(getTemporalBarClass("2026-07-06", TODAY)).toBe("w-[3px] bg-cyan-accent/40");
  });
